...
export class OverlaysService {
  ...
  async create(dto: CreateOverlayDto): Promise<Overlay> {
    const overlay = await this.configLayerRepo.createOverlay({
  ...
  async findAll(query: ListOverlaysQueryDto): Promise<Overlay[]> {
    return this.configLayerRepo.findOverlaysByWorkspace(
      query.workspaceId,
      {
  ...
  async findOne(id: string): Promise<Overlay> {
    const overlay = await this.configLayerRepo.findOverlayById(id);
  ...
  async update(id: string, dto: UpdateOverlayDto): Promise<Overlay> {
    ...
    return this.configLayerRepo.updateOverlay(id, {
  ...
  async remove(id: string): Promise<void> {
    await this.findOne(id);
    await this.configLayerRepo.deleteOverlay(id);
  }
  ...
  async findApplicableOverlays(workspaceId: string, filters: {
    ...
    const overlays = await this.configLayerRepo.findOverlaysByWorkspace(
      workspaceId,
      { enabled: true },
    );
